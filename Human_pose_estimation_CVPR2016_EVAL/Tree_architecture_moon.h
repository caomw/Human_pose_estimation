		...
		for (int f = 0; f < TEST_FRAME_NUM; f++)
		{
			...
			for (int i = 0; i < Q_HEIGHT * Q_WIDTH; i++)
			{
				...
				for (int tid = 0; tid < test_tree_number; tid++)
				{
					...
					for (int nid = 0; nid < NEAREST_NUM; nid++)
					{
                        if(sqrt(pow(estimated_offset[nid][0],2) + pow(estimated_offset[nid][1],2) + pow(estimated_offset[nid][2],2) < 0.1))
                        {
                        effective_tn[nid]++;
                ...
                for (int nid = 0; nid < NEAREST_NUM; nid++)
				{
                    if(effective_tn[nid] > 0)
                    {
                    average_offset[nid][0] /= (float)effective_tn[nid];
					average_offset[nid][1] /= (float)effective_tn[nid];
					average_offset[nid][2] /= (float)effective_tn[nid];
                    }
				}
            ...
            //mean_shift for clustering
			clock_t cluster_begin, cluster_end;
			cluster_begin = clock();
            int *is_erased;
            is_erased = new int[sample.rows];
            for(int i=0; i<sample.rows; i++)
                is_erased[i] = 0;
             
            for(int jid=0; jid < CLUSTER_JOINT_NUMBER; jid++)
            {
            float average_pt[3];
            average_pt[0] = average_pt[1] = average_pt[2] = 0;
            for(int i=0; i<sample.rows; i++)
            {
                if(is_erased[i] == 0)
                {
                average_pt[0] += sample.at<float>(i,0);
                average_pt[1] += sample.at<float>(i,1);
                average_pt[2] += sample.at<float>(i,2);
                }
            }
            average_pt[0] /= (float)sample.rows;
            average_pt[1] /= (float)sample.rows;
            average_pt[2] /= (float)sample.rows;
            
            float *prev_mean;
            prev_mean = new float[3];
            prev_mean[0] = average_pt[0];
            prev_mean[1] = average_pt[1];
            prev_mean[2] = average_pt[2];

            for(float rad=2; rad>=0.0; rad-=0.1)
            {
                int pt_num = 0;
                float new_mean[3];
                new_mean[0] = new_mean[1] = new_mean[2] = 0;
                for(int i=0; i<sample.rows; i++)
                {
                    float dist = sqrt(pow(prev_mean[0]-sample.at<float>(i,0),2) + pow(prev_mean[1]-sample.at<float>(i,1),2) + pow(prev_mean[2]-sample.at<float>(i,2),2));
                    if(is_erased[i] == 0 && dist < rad)
                    {
                        if(rad <= 0.2)
                            is_erased[i] = 1;
                        pt_num++;
                        new_mean[0] += sample.at<float>(i,0);
                        new_mean[1] += sample.at<float>(i,1);
                        new_mean[2] += sample.at<float>(i,2);
                    }
                }

                if(pt_num > 0)
                {
                new_mean[0] /= (float)pt_num;
                new_mean[1] /= (float)pt_num;
                new_mean[2] /= (float)pt_num;
                
                prev_mean[0] = new_mean[0];
                prev_mean[1] = new_mean[1];
                prev_mean[2] = new_mean[2];
                }
            }
            clustered_points.push_back(prev_mean);
            }
			...
			for(int iter = 0; iter < sample.rows; iter++)
            {
