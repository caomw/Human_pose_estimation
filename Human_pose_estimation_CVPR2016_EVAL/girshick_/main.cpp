...
int main()
{
	...
	cout << "Developed by Gyeongsik Moon and Ho Yub Jung" << endl << endl;
	...
	if (!is_save.compare("SAVE"))
	{
		...
		for (long long int pid = 0; pid < TRAIN_FRAME_NUM * Q_HEIGHT * Q_WIDTH; pid++)
		{
			if (point_train[pid].depth != BACKGROUND_DEPTH)
			{
				...
				delete [] point_train[pid].offset_id;
	...
	point_matching = new point[POINT_MATCHING_FRAME_NUM * Q_HEIGHT * Q_WIDTH]; // this has memory problem //maybe we should load indivisually...
	...
	tree_traversal_girshick(point_test, joint_test, center_matching, joint_matching, tree_head);
	end = clock();
