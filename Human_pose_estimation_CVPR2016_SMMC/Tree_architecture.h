...
#define WINDOW 1
#define TRAINING_DIR "D:\\Study/Data/Human_pose_estimation/SMMC/"
#define SAVE_DIR "D:\\Study/Data/Human_pose_estimation/Result_SMMC/"
...
#define THR 0.1
#define RANDOM_SAMPLE 600
#define NN_SEARCH_RADIUS 0.1f

#define TRAIN_FRAME_NUM 1374
#define POINT_MATCHING_FRAME_NUM 1374
#define TEST_FRAME_NUM 5500
#define FRAME_INTERVAL 5
#define TRAIN_REMAIN 0
...
void split_node(node* tree_ptr, int depth, int& node_number, point* point_train, int& leaf_number, int tree_number)
{
	...
	//if (node_number >= MAX_NODE_NUM || tree_ptr->sample.size() < MIN_SAMPLE_NUM)
...
{
	for (int test_tree_number = 1; test_tree_number <= tree_head.size(); test_tree_number+=3)
	{
		node* tree_ptr;
		stringstream ss;
		...
		for (int f = 0; f < TEST_FRAME_NUM; f++)
		{
			...
			float max_pt[3];
			float min_pt[3];
			max_pt[0] = max_pt[1] = max_pt[2] = -9999;
			min_pt[0] = min_pt[1] = min_pt[2] = 9999;
			...
			for (int i = 0; i < Q_HEIGHT * Q_WIDTH; i++)
			{
				...
				{
					if (max_pt[0] < point_test[f * Q_HEIGHT * Q_WIDTH + i].x_world)
						max_pt[0] = point_test[f * Q_HEIGHT * Q_WIDTH + i].x_world;
					if (max_pt[1] < point_test[f * Q_HEIGHT * Q_WIDTH + i].y_world)
						max_pt[1] = point_test[f * Q_HEIGHT * Q_WIDTH + i].y_world;
					if (max_pt[2] < point_test[f * Q_HEIGHT * Q_WIDTH + i].depth)
						max_pt[2] = point_test[f * Q_HEIGHT * Q_WIDTH + i].depth;

					if (min_pt[0] > point_test[f * Q_HEIGHT * Q_WIDTH + i].x_world)
						min_pt[0] = point_test[f * Q_HEIGHT * Q_WIDTH + i].x_world;
					if (min_pt[1] > point_test[f * Q_HEIGHT * Q_WIDTH + i].y_world)
						min_pt[1] = point_test[f * Q_HEIGHT * Q_WIDTH + i].y_world;
					if (min_pt[2] > point_test[f * Q_HEIGHT * Q_WIDTH + i].depth)
						min_pt[2] = point_test[f * Q_HEIGHT * Q_WIDTH + i].depth;
			...
			int effective_pixel_number = 0;
			vote vote_image[Q_HEIGHT * Q_WIDTH];
			for (int i = 0; i < Q_HEIGHT * Q_WIDTH; i++)
			{
				vote_image[i].x_pixel = i % Q_WIDTH;
				vote_image[i].y_pixel = i / Q_WIDTH;
				vote_image[i].depth = point_test[f * Q_HEIGHT * Q_WIDTH + i].depth;
				vote_image[i].priority = 0;
			}
			...
			for (int i = 0; i < Q_HEIGHT * Q_WIDTH; i++)
			{
				...
				////draw heat map
				//sort(vote_image, vote_image + Q_HEIGHT * Q_WIDTH);
				//int effective_pixel_index = 0;
				//for (int effective_pixel_index = 0; effective_pixel_index < effective_pixel_number;)
				//{
				//	
				//if (vote_image[effective_pixel_index].priority > 0)
				//{
				//int g = ((float)effective_pixel_index / (float)effective_pixel_number) * (float)255;
				//int r = 255 - g;C

				//rectangle(display, Rect(vote_image[effective_pixel_index].x_pixel, vote_image[effective_pixel_index].y_pixel, 1, 1), CV_RGB(r, g, 0), 2);
				//effective_pixel_index++;
				//}
				//}
				...
				kmeans(sample, CLUSTER_JOINT_NUMBER, best_label, TermCriteria(CV_TERMCRIT_EPS + CV_TERMCRIT_ITER, K_MEANS_ITER, K_MEANS_EPS), K_MEANS_ATTEMPT, KMEANS_PP_CENTERS, center);
