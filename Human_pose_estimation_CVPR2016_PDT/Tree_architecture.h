...
#define TRAINING_DIR "D:\\Study/Data/Human_pose_estimation/PDT/"
#define SAVE_DIR "D:\\Study/Data/Human_pose_estimation/Result_PDT/"
#define Q_WIDTH 640
#define Q_HEIGHT 480
#define BACKGROUND_DEPTH 100
#define COORDINATE_DIM 3
#define JOINT_NUMBER 15
#define RESULT_JOINT_NUMBER 12
#define SEARCH_RADIUS_XY 2.0f
#define SEARCH_RADIUS_Z 0.4f
#define SHOULDER_LENGTH 0.8f
...
//F1D1: 1400, F1D2: 1632, F1D3: 910, F1D4: 1218
//F2D1: 1562, F2D2: 1375, F2D3: 1404, F2D4: 1403
//M1D1: 996, M1D2: 1412, M1D3: 1193, M1D4: 1412
//M2D1: 1094, M2D2: 1050, M2D3: 1174, M2D4: 1278
//M3D1: 1402, M3D2: 2045, M3D3: 1518, M3D4: 1523
#define TRAIN_FRAME_NUM 2000
#define POINT_MATCHING_FRAME_NUM 2000
#define TEST_FRAME_NUM 1402
#define TRAIN_MODEL_1 "F1"
#define TRAIN_MODEL_2 "F2"
#define TRAIN_MODEL_3 "M1"
#define TRAIN_MODEL_4 "M2"
#define TEST_MODEL "M3"
#define TEST_DIFFICULTY "D1"
...
void split_node(node* tree_ptr, int depth, int& node_number, point* point_train, int& leaf_number)
{
	cout << node_number << "th node is in training... ";
	...
	else
	{
		...
		split_node(tree_ptr->left_child, depth + 1, node_number, point_train, leaf_number);
		split_node(tree_ptr->right_child, depth + 1, node_number, point_train, leaf_number);
...
void tree_train(point* point_train, joint** joint_train, vector<node*>& tree_head)
{
	...
	int node_number = 0;
	int leaf_number = 0;
	split_node(tree_ptr, 0, node_number, point_train, leaf_number);
...
void tree_traversal(point* point_test, joint** joint_test, float** center_train, joint** joint_train, vector<node*> tree_head)
{
	...
	ss << SAVE_DIR << "average_result.txt";
	...
	ss << SAVE_DIR << "upper_bound_result.txt";
	...
	for (int f = 0; f < TEST_FRAME_NUM; f++)
	{
		...
		for (int i = 0; i < Q_HEIGHT * Q_WIDTH; i++)
		{
			...
			for (int nid = 0; nid < NEAREST_NUM; nid++)
			{
				...
				pt[0] = point_test[f * Q_HEIGHT * Q_WIDTH + i].x_world + estimated_offset[nid][0];
				pt[1] = point_test[f * Q_HEIGHT * Q_WIDTH + i].y_world + estimated_offset[nid][1];
				pt[2] = point_test[f * Q_HEIGHT * Q_WIDTH + i].depth + estimated_offset[nid][2];
				estimated_sample.at<float>(0, 0) = pt[0];
				estimated_sample.at<float>(0, 1) = pt[1];
				estimated_sample.at<float>(0, 2) = pt[2];
		...
		Mat best_label, center;
		kmeans(random_sample, JOINT_NUMBER, best_label, TermCriteria(CV_TERMCRIT_EPS + CV_TERMCRIT_ITER, K_MEANS_ITER, K_MEANS_EPS), K_MEANS_ATTEMPT, KMEANS_PP_CENTERS, center);
		...
		for (int jid = 0; jid < JOINT_NUMBER; jid++)
		{
			...
			if (jid == HEAD)
			{
				...
				//putText(display, ss.str(), Point(pt[0] - 2, pt[1] - 2), 1, 0.8, Scalar(0, 0, 255));
		...
		for (int jid = 0; jid < JOINT_NUMBER; jid++)
		{
			if (jid != L_HIPS && jid != R_HIPS && jid != BLLY)
			{
				for (int cid = 0; cid < clustered_points.size(); cid++)
				{
					float dist = sqrt(pow(joint_test[f][jid].x_world - clustered_points[cid][0], 2) + pow(joint_test[f][jid].y_world - clustered_points[cid][1], 2) + pow(joint_test[f][jid].depth - clustered_points[cid][2], 2));
