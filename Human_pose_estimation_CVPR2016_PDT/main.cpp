...
int main()
{
	...
	if (!is_save.compare("SAVE"))
	{
        ...
        point_train = new point[TRAIN_FRAME_NUM*Q_WIDTH*Q_HEIGHT];
	...
	else if (!is_save.compare("LOAD"))
	{
        ...
        for(int fid=0; fid<POINT_MATCHING_FRAME_NUM; fid++)
            joint_matching[fid] = new joint[JOINT_NUMBER];
        ...
        for(int fid=0; fid<TEST_FRAME_NUM; fid++)
            joint_test[fid] = new joint[JOINT_NUMBER];
